...
/// <summary>
/// A short magic-circle summon centered independently on every struck creature.
/// </summary>
public partial class NBingJingMoZhenVfx : Node2D
{
    public static readonly string ScenePath = $"{VFXUtil.CardVfxPath}/vfx_BingJingMoZhen.tscn";
    ...
    public static NBingJingMoZhenVfx? Create(Creature? target)
    {
        if (TestMode.IsOn)
        {
            return null;
        ...
        NBingJingMoZhenVfx vfx = VFXUtil.GenVFXNode<NBingJingMoZhenVfx>(ScenePath);
    ...
    {
        // Rotation uses one uninterrupted linear tween, so its angular speed is constant.
        ...
        // Animate shader uniforms directly: the shader writes COLOR itself, so parent
        // CanvasItem modulation is not a dependable way to produce a strong fade.
