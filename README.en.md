...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/Shuyu/wiki/Home) | [English Home](https://github.com/alkaid616/Shuyu/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `Shuyu`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `Shuyu.json` and update `id`, `name`, `author`, and `description`.
3. Edit `ShuyuCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `Shuyu/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `Shuyu.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `Shuyu.json` has been synced to the resolved `STS2.RitsuLib` version.
...
Shuyu/
├── ShuyuCode/   # C# source
├── Shuyu/       # Godot resources, localization, and placeholder scenes
├── Shuyu.csproj
├── Shuyu.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://Shuyu/...` is the Godot/PCK resource path, mapping to the repository resource directory `Shuyu/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
|---|---|
| Type | `ShuyuCharacter` |
| Expected ID | `SHUYU_CHARACTER_SHUYU_CHARACTER` |
| Starter deck | 4 × `ShuyuStrike`, 4 × `ShuyuDefend`, 1 × `ShuyuRelic` |
...
|---|---|---|
| `ShuyuStrike` (attack) | character card pool | `SHUYU_CARD_SHUYU_STRIKE` |
| `ShuyuDefend` (skill) | character card pool | `SHUYU_CARD_SHUYU_DEFEND` |
| `ShuyuRelic` | `ShuyuRelicPool` | `SHUYU_RELIC_SHUYU_RELIC` |

### Static placeholder assets

**Images** (`res://Shuyu/images/...`):

- `cards/ShuyuStrike.png`, `cards/ShuyuDefend.png`: sample card art.
- `relics/ShuyuRelic.png`: sample relic icon.
- `characters/Shuyu_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://Shuyu/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `Shuyu_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `Shuyu_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `Shuyu_merchant.tscn` | Merchant | — |
| `Shuyu_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `Shuyu_character_select_bg.tscn` | Character select background | — |
...
`Shuyu.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
